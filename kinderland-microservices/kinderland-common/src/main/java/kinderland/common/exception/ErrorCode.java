...
public enum ErrorCode {
    ...
    PROMOTION_EXPIRED("Mã khuyến mãi đã hết hạn", HttpStatus.CONFLICT),
    PROMOTION_NOT_STARTED("Mã khuyến mãi chưa tới thời gian áp dụng", HttpStatus.CONFLICT),
    PROMOTION_INACTIVE("Mã khuyến mãi đang bị khoá", HttpStatus.CONFLICT),
    PROMOTION_USAGE_LIMIT_REACHED("Mã khuyến mãi đã hết lượt sử dụng", HttpStatus.CONFLICT),
    PROMOTION_MIN_ORDER_NOT_MET("Đơn hàng chưa đạt giá trị tối thiểu để áp mã khuyến mãi", HttpStatus.CONFLICT),
    PROMOTION_INVALID("Mã khuyến mãi không hợp lệ", HttpStatus.BAD_REQUEST),
    ...
    BRAND_IN_USE("Không thể xoá: còn sản phẩm thuộc thương hiệu này (kể cả sản phẩm trong thùng rác)", HttpStatus.CONFLICT),
    CATEGORY_IN_USE("Không thể xoá: còn sản phẩm thuộc danh mục này (kể cả sản phẩm trong thùng rác)", HttpStatus.CONFLICT),
